...
var fornecedorMapaOrdenacao = { coluna: "previsto", direcao: "asc" };
...
function garantirFornecedoresEdicaoPedido() {
    ...
    if (!__fornecedoresEdicaoPromessa) {
        __fornecedoresEdicaoPromessa = carregarScriptAdmin("fornecedores-edicao-pedido.js?v=20260908-ref-preco-fornecedor");
    }
...
function garantirFornecedoresPrintReceive() {
    ...
    if (!__fornecedoresPrintPromessa) {
        __fornecedoresPrintPromessa = carregarScriptAdmin("fornecedores-print-receive.js?v=20260908-ref-preco-fornecedor");
    }
...
function fundirItemPedidoFornecedor(destino, origem) {
    ...
    if (!destino.data_origem_ajuste && origem.data_origem_ajuste) {
        destino.data_origem_ajuste = origem.data_origem_ajuste;
    }
    return destino;
}
...
function serializarItemPedidoFornecedor(item) {
    ...
    return {
        ...
        data_origem_ajuste: normalizado.data_origem_ajuste || null,
...
function formatarDataAjustePedidoFornecedor(valor) {
    if (!valor) return "";
...
function obterTextoOrigemAjusteItemPedidoFornecedor(item, pedido = null) {
    const origemAjuste = String(item?.origem_ajuste || "").trim();
    if (origemAjuste === "substituicao") {
        const data = formatarDataAjustePedidoFornecedor(
            item?.data_origem_ajuste || pedido?.atualizado_em || pedido?.criado_em
        );
        return data || obterTextoOrigemAjustePedidoFornecedor(origemAjuste);
    }
...
function criarBlocoHistoricoFornecedorFicha(form, id, rotulo, valor, opcoes = {}) {
    ...
    const precoCompraTexto = obterTextoPrecoCompraMarcacaoFornecedor(valor);
    if (precoCompraTexto) {
        const precoCompra = document.createElement("span");
        precoCompra.className = "mapas-produto-fornecedor-preco-compra";
        precoCompra.textContent = precoCompraTexto;
...
function criarItemFornecedorAPartirListaFinal(analisada, produto = null) {
    ...
    return normalizarItemPedidoFornecedor({
        ...
        data_origem_ajuste: dataOsAgoraFornecedor(),
        recebido: 0,
        preco_custo: precoCusto,
        preco: precoCusto,
        sem_catalogo: true
...
function criarItemFornecedorAPartirSelecao(item, origemAjuste = '') {
    ...
    const origem = String(origemAjuste || '').trim();
    return normalizarItemPedidoFornecedor({
        id: item.id,
        nome: item.nome,
        sku: item.sku || '',
        ...
        data_origem_ajuste: origem ? dataOsAgoraFornecedor() : null,
        recebido: 0,
        novidade: obterNovidadeParaItemPedidoFornecedor(item),
        stock_no_momento: Number(item.stock || 0),
        preco_custo: Number(item.preco_custo ?? item.custo ?? 0) || 0,
...
async function adicionarSelecaoAoPedidoFornecedor(id) {
    ...
    fornecedorSelecao.forEach(selecionado => {
        ...
        if (existente) {
            ...
            existente.data_origem_ajuste = existente.data_origem_ajuste || dataOsAgoraFornecedor();
...
function renderizarPedidoFornecedorProdutosTabela(caixa, pedido) {
    ...
    ordenarItensPedidoFornecedor(pedido.itens || []).forEach(item => {
        const indiceOriginal = (pedido.itens || []).indexOf(item);
        const produtoAtual = obterProdutoParaPedidoFornecedor(item) || item;
        ...
        nomeBotao.type = "button";
        nomeBotao.className = "mapas-produto-nome-botao";
        nomeBotao.textContent = produtoAtual.nome || item.nome || "Produto sem nome";
        nomeBotao.title = "Editar produto";
        nomeBotao.tabIndex = -1;
        if (produtoAtual?.id) {
        ...
        input.dataset.indice = String(indiceOriginal);
...
function criarDetalhesPedidoFornecedor(pedido) {
    ...
    if (estaPaginaFornecedoresUnificada()) {
        ...
        pedido.itens.forEach((item, indice) => {
            ...
            info.innerHTML = `<strong>${escaparHtmlFornecedor(produtoAtual.nome || item.nome)}</strong><span class="fornecedor-identificadores">Ref. ${escaparHtmlFornecedor(produtoAtual.referencia || item.referencia || "-")} | SKU ${escaparHtmlFornecedor(produtoAtual.sku || item.sku || "-")}</span><span>Pedido: ${Number(item.quantidade || 0)} | Recebido: ${recebido} | Stock atual: ${Number(produtoAtual.stock || 0)}</span>${faltaOs > 0 ? `<span class="fornecedor-ajuste-os ativo">OS/Falta: ${faltaOs}${item.quantidade_original ? ` de ${Number(item.quantidade_original || 0)}` : ""}</span>${dataOs ? `<span class="fornecedor-ajuste-os">${escaparHtmlFornecedor(dataOs)}</span>` : ""}` : ""}${marcadoEx ? `<span class="fornecedor-ajuste-os fornecedor-ajuste-ex ativo">EX</span>` : ""}${textoOrigemVisivel ? `<span class="fornecedor-ajuste-os">${escaparHtmlFornecedor(textoOrigemVisivel)}</span>` : ""}`;
            const input = document.createElement("input");
            input.type = "number";
            input.min = "0";
            input.max = String(restante);
