...
function criarCardFavoritoCliente(produto) {
    ...
    imagem.src = otimizarImagemCloudinary(obterImagemPrincipalProduto(produto), 140);
...
async function renderizarFavoritosCliente() {
    ...
    if(!ids.length) {
        definirFavoritosVazio('Ainda não tem favoritos guardados.');
        return;
    }

    lista.replaceChildren();
    const carregamento = document.createElement('p');
    carregamento.className = 'historico-vazio';
